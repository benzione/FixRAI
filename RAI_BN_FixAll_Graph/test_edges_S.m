...
%         par=mysetdiff(pa,P(k));
        ...
        pa1 = find(Gend(:,C(k)));% identify the parents of the child node % G->Gend
        par1=mysetdiff(pa1,P(k));
        
        cChild=mysetdiff(par,par1');
...
%         par=mysetdiff(pa,P(k));
        ...
        S = subsets1(sort(par1),n); % select a condition set with size n from the parents group
    else
        S = {[]};
    end
    for si = 1:length(S)
...
%                 [CI, ~]=mutual2_decision(P(k),C(k), [S{si},class_node], data,threshold_val);
                ...
                if CI
                    if ~isempty(S{si})
                        if ~isempty(myintersect(cChild,S{si}))
                            if (fid>0) && (n>0) % write to file
                                fprintf(fid,'%d,%d,%d',P(k),C(k),n);
                                fprintf(fid,',%d',length(S{si}));
                                if ~isempty(S{si})
                                    str='';
                                    for t=1:length(S{si})
                                        str=strcat(str,',',num2str(S{si}(t)));
                                    end
                                    fprintf(fid,str);
                                end
                                fprintf(fid,',%d',length(cChild));
                                if ~isempty(cChild)
                                    str='';
                                    for t=1:length(cChild)
                                        str=strcat(str,',',num2str(cChild(t)));
                                    end
                                    fprintf(fid,str);
                                end
                                fprintf(fid,',%d',length(s_nodes));
                                if ~isempty(s_nodes)
                                    str='';
                                    for t=1:length(s_nodes)
                                        str=strcat(str,',',num2str(s_nodes(t)));
                                    end
                                    fprintf(fid,str);
                                end
                                fprintf(fid,'\n');                                
%                                 fprintf(fid,',%d,%d\n',length(ex_nodes),ex_nodes);
%                                 fprintf(fid,',%d,%d\n',length(s_nodes),s_nodes);
