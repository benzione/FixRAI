function pdag=create_cpdag1(G,sep)
n = size(G,1); % number of nodes

sfname1='C:\Users\benzione\Dropbox\Research\RAI\RAI\RAI DBN\RAI_BN_FixAll\fix1.txt';
fid1=fopen(sfname1,'at');
sfname2='C:\Users\benzione\Dropbox\Research\RAI\RAI\RAI DBN\RAI_BN_FixAll\fix2.txt';
fid2=fopen(sfname2,'at');
    ...
    for j=1:lengthX-1
        for k=j+1:lengthX
            ...
            if (G(x,z)==0) && (G(z,x)==0)...
                ...
                fprintf(fid1,'%d,%d,%d',i,x,z);
                fprintf(fid1,',%d',length(sep{x,z}));
                if ~isempty(sep{x,z})
                    str='';
                    for t=1:length(sep{x,z})
                        str=strcat(str,',',num2str(sep{x,z}(t)));
                    end
                    fprintf(fid1,str);
                end
                fprintf(fid1,',%d',length(sep{z,x}));
                if ~isempty(sep{z,x})
                    str='';
                    for t=1:length(sep{z,x})
                        str=strcat(str,',',num2str(sep{z,x}(t)));
                    end
                    fprintf(fid1,str);
                end
                fprintf(fid1,'\n');
...
[X, Y] = find(G==1);
pdag = ((G==1) & ((G'==1))); % work only on undirected edges
...
%     if (pdag(y,z) == -1), fprintf('%d->%d was reversed\n',y,z);end
      if ~isempty(check)
        fprintf(fid2,'%d,%d,%d',y,x,z);
        fprintf(fid2,',%d',length(sep{x,z}));
        if ~isempty(sep{x,z})
            str='';
            for t=1:length(sep{x,z})
                str=strcat(str,',',num2str(sep{x,z}(t)));
